...
Output (graph.json):
  attrs: [[kind, lim, name, flags, (dates)], ...]   flags: 1 closed, 2 seasonal, 4 military, 8 connector, 16 high clearance, 32 4x4 + high clearance
...
def build_graph(trail_features, road_features, out_path):
    ...
    for f in trail_features + road_features:
        p = f["properties"]
        if p["t"] not in ("route", "trail", "mc", "mccct", "road", "nf"):
            continue
        ...
        # national forest roads carry their open dates ("05/16-03/14") so the phone can skip ones closed today
        a = attr_id((p["t"], p.get("lim") or 999, p.get("n") or "", flags) + ((p["dates"],) if p.get("dates") else ()))
        g = f["geometry"]
        ...
        for s, e in zip(bounds, bounds[1:]):
            if e - s > 1e-9:
                ...
                if closure_zone is not None and not attrs[a][3] & 1 and closure_zone.intersects(piece)                         and piece.length > 0 and piece.intersection(zone_geom).length / piece.length > 0.6:
                    kind, lim, name, flags, *rest = attrs[a]
                    use = attr_id((kind, lim, name, flags | 1, *rest))
                    closed_extra += 1
                emit(piece, use)
