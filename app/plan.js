...
function alongTrip(kind, maxOff) {
  ...
  let offset = 0;
  for (const r of planLegs) {
    if (!r.path.length) { offset += r.meters; continue; }
    ...
    for (const p of POIS) {
      if (p.t !== kind || p.lat < box[0] || p.lat > box[1] || p.lng < box[2] || p.lng > box[3]) continue;
      const kx = Math.cos(p.lat * Math.PI / 180) * 111320, ky = 110540;
      let best = Infinity, at = 0;
      for (let i = 0; i < r.path.length - 1; i++) {
        const [ay, ax] = r.path[i], [by, bx] = r.path[i + 1];
        const dx = (bx - ax) * kx, dy = (by - ay) * ky, px = (p.lng - ax) * kx, py = (p.lat - ay) * ky;
        const L2 = dx * dx + dy * dy;
        const t = L2 ? Math.max(0, Math.min(1, (px * dx + py * dy) / L2)) : 0;
        const d = Math.hypot(px - dx * t, py - dy * t);
        if (d < best) { best = d; at = r.cum[i] + (r.cum[i + 1] - r.cum[i]) * t; }
      }
      if (best <= maxOff) {
        const cur = hits.get(p);
        if (!cur || best < cur.off) hits.set(p, { p, off: best, at: offset + at });
      }
    }
    offset += r.cum[r.cum.length - 1] || r.meters;
  }
  return [...hits.values()].sort((a, b) => a.at - b.at);
}
...
function showPlan() {
  ...
  if (planLegs.length) {
    const gas = alongTrip('gas', GAS_NEAR_M);
    const total = t.m;
...
function updateBar() {
  ...
  bar.querySelector('span').textContent = n < 2 ? '1 stop · add more'
    : `${fmtMi(t.m)}${gap ? ' · has gaps' : ' · ~' + fmtTime(t.s)}${n > 2 ? ` · ${n} stops` : ''}`;
}
